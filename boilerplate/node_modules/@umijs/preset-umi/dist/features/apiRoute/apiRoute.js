...
exports.default = (api) => {
    ...
    api.addBeforeMiddlewares(() => [
        async (req, res, next) => {
            if (req.path.startsWith('/api')) {
                ...
                await require((0, path_1.join)(api.paths.cwd, constants_2.OUTPUT_PATH, matchedApiRoute.route.file).replace('.ts', '.js')).default(req, res);
                return;
            }
            next();
        },
    ...
    api.onBeforeCompiler(async () => {
        ...
        const apiRoutes = Object.keys(api.appData.apiRoutes).map((k) => api.appData.apiRoutes[k]);
        if (api.env === 'development') {
            await (0, esbuild_1.default)(api, apiRoutes);
            return;
        }
        if (fs_1.default.existsSync((0, path_1.join)(api.paths.cwd, constants_2.OUTPUT_PATH))) {
            await fs_1.default.rmdirSync((0, path_1.join)(api.paths.cwd, constants_2.OUTPUT_PATH), {
                recursive: true,
            });
        }
        switch (platform) {
            case ServerlessPlatform.Vercel:
                await (0, esbuild_2.default)(api, apiRoutes);
                return;
